...
export default function TestsPage() {
  ...
  return (
    <main className="page section">
      ...
      <div className="product-grid">{catalog.map(x=><article className="product-card clickable-card" role="link" tabIndex={0} aria-label={`Explorar: ${x.name}`} onClick={()=>navigate(x.free_code?`/test/${x.free_code}`:'/tests/vocacional-ia-2026')} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();navigate(x.free_code?`/test/${x.free_code}`:'/tests/vocacional-ia-2026')}}} key={x.type_code}>
        <div className="module-icon">{x.icon}</div><h2>{x.name}</h2><p>{x.description}</p>
        <ul>{x.free_code&&<li>{x.free_questions} preguntas gratis</li>}<li>{x.premium_questions} preguntas avanzadas</li><li>Resultado inmediato</li></ul>
        <div className="card-actions">{x.free_code?<Link onClick={e=>e.stopPropagation()} className="btn secondary" to={`/test/${x.free_code}`}>Comenzar gratis</Link>:<Link onClick={e=>e.stopPropagation()} className="btn secondary" to="/tests/vocacional-ia-2026">Conocer el test</Link>}<Link onClick={e=>e.stopPropagation()} className="btn primary" to={`/acceso/${x.premium_code}`}>Avanzado · {x.price} {x.currency}</Link></div>
      </article>)}</div>
    </main>
  )
}
