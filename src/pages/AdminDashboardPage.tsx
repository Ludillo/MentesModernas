...
export default function AdminDashboardPage() {
  ...
  const navItems:[Tab,string][]=[
    ...
    ['visits','Visitas'],['coupons','Cupones'],['users','Usuarios y accesos'],...(session?.admin.role==='SUPERADMIN'?([['administrators','Administradores']] as [Tab,string][]):[]),['tests','Configuración de pagos y tests'],['reports','Reportes'],['security','Seguridad']
  ]
  ...
  const createCoupon=async()=>{
    ...
    const productCode=prompt('Código del producto (ver Configuración de pagos y tests)','VOCATIONAL_AI_2026_PREMIUM')
    if(!productCode)return
    try{await adminApi('coupon-create',{code,discountPercent:discount,maxUses,productCode})}catch(e:any){setError(e.message);return}
    await load('coupons')
  }
  ...
  return (
    <main className="admin-shell">
      ...
      <section className="admin-content">
        <div className="admin-top"><div><span className="eyebrow">ADMIN</span><h1>{navItems.find(([key])=>key===tab)?.[1]??tab}</h1></div><span>{session.admin.email}</span></div>
...
function ProductPrices({items,onSaved}:{items:any[],onSaved:()=>Promise<void>}){
 ...
 return <div className="admin-card"><h2>Precios de tests avanzados</h2><p className="admin-hint">Configura el valor de cobro por cada test. Moneda: bolivianos (BOB).</p>{products.map((p:any)=><div className="admin-inline-input" key={p.code}><label>{p.name}<input type="number" min="1" step="0.01" value={values[p.code]??p.price} onChange={e=>setValues(current=>({...current,[p.code]:e.target.value}))}/></label><button className="btn secondary" onClick={async()=>{try{await adminApi('product-price-update',{productCode:p.code,price:Number(values[p.code]??p.price),currency:'BOB'});setMsg(`Precio de ${p.name} actualizado.`);await onSaved()}catch(e:any){setMsg(e.message)}}}>Guardar precio</button></div>)}{msg&&<div className="alert">{msg}</div>}</div>
}
function PaymentsPanel({items,onRefresh}:{items:any[],onRefresh:()=>Promise<void>}){
  const [busy,setBusy]=useState('');const [msg,setMsg]=useState('');const [syncing,setSyncing]=useState(false);const [syncDate,setSyncDate]=useState(()=>new Date().toISOString().slice(0,10))
  ...
  const reconcile=async()=>{setSyncing(true);setMsg('');try{const result=await adminApi('qr-reconcile',{date:syncDate});setMsg(result.message??'Sincronización terminada.');await onRefresh()}catch(e:any){setMsg(e.message)}finally{setSyncing(false)}}
  ...
  return <><div className="admin-section-head"><div><h2>Pagos pendientes</h2><p className="admin-hint">Selecciona la fecha del pago. Se obtiene un único reporte de Banco Económico y se cruzan localmente sus identificadores con todos los QR pendientes.</p></div><div className="payment-sync-actions"><span className="pending-badge">{pending.length} pendientes</span><label>Fecha del reporte<input type="date" value={syncDate} onChange={e=>setSyncDate(e.target.value)}/></label><button className="btn primary" disabled={syncing||!pending.some(x=>x.provider_transaction_id)} onClick={reconcile}>{syncing?'Consultando un reporte…':'Sincronizar pagos QR'}</button></div></div>{msg&&<div className="alert">{msg}</div>}{pending.length?list(pending):<div className="admin-card"><p>No hay pagos pendientes de revisión.</p></div>}<h2 className="admin-subtitle">Historial de pagos</h2>{reviewed.length?list(reviewed):<p className="admin-hint">Todavía no hay pagos revisados.</p>}</>
}
...
function SecurityPanel(){return <div className="admin-card"><h2>Seguridad administrativa</h2><p>El acceso utiliza tu cuenta Google. Administra su contraseña y verificación en dos pasos desde Google.</p><a className="btn secondary" href="https://myaccount.google.com/security" target="_blank" rel="noreferrer">Seguridad de mi cuenta Google</a><p>Los superadministradores pueden gestionar las cuentas autorizadas en la sección Administradores.</p></div>}
