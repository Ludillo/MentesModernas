...
export default function GenericTestPage(){
 ...
 if(result.length)return <main className="page section"><div className="page-hero compact"><span className="eyebrow">RESULTADO</span><h1>Tu perfil principal: {result[0].name}</h1><p>Este resultado orienta tu reflexión y no constituye un diagnóstico clínico.</p></div><div className="areas-result-grid">{result.map(x=><article className="score-card" key={x.code}><h2>{x.name}</h2><strong>{x.percent}%</strong><div className="score-bar"><i style={{width:`${x.percent}%`}}/></div></article>)}</div></main>
 if(!questions.length)return <main className="page section"><div className="loading">Preparando evaluación…</div></main>
 const finish=async(answers:Record<string,number>)=>{const groups=new Map<string,{total:number,count:number}>();questions.forEach(q=>{const g=groups.get(q.dimension_code)||{total:0,count:0};g.total+=answers[q.id]||0;g.count++;groups.set(q.dimension_code,g)});const results=[...groups].map(([code,g])=>({code,name:code.replaceAll('_',' '),score:g.total,maxScore:g.count*4,percent:Math.round(g.total/(g.count*4)*100),description:'',careers:[]})).sort((a,b)=>b.percent-a.percent);if(code.endsWith('_PREMIUM')){const saved=await submitPremiumResult({testCode:code,answers,results:results as any});navigate(`/resultado/${saved.evaluationId}`)}else setResult(results)}
