...
export default function GenericTestPage(){
 ...
 const finish=async(answers:Record<string,number>)=>{const groups=new Map<string,{total:number,count:number}>();questions.forEach(q=>{const g=groups.get(q.dimension_code)||{total:0,count:0};g.total+=answers[q.id]||0;g.count++;groups.set(q.dimension_code,g)});const results=[...groups].map(([dimension,g])=>({code:dimension,name:dimension.replaceAll('_',' '),score:g.total,maxScore:g.count*4,percent:Math.round(g.total/(g.count*4)*100),description:'',careers:[]})).sort((a,b)=>b.percent-a.percent);recordTestCompletion(code).catch(()=>{});if(code.endsWith('_PREMIUM')){const saved=await submitPremiumResult({testCode:code,answers,results:results as any});navigate(`/resultado/${saved.evaluationId}`)}else setResult(results)}
